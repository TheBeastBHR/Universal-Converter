...
//****NOTE:this is an old test runner kept just in case****
...
class UnitConverterTester {
  ...
  // Test for double detection prevention (bug fix verification)
  testDoubleDetectionPrevention() {
    console.log(`\n${colors.blue}[Testing Double Detection Prevention]${colors.reset}`);
    ...
    // Test case 1: Dimension format that previously caused double detection
    console.log('Testing dimension format: "36.5 x 110.8 x 32 cm"');
    const dimensionText = 'Product size: 36.5 x 110.8 x 32 cm';
    const allConversions = this.detector.findConversions(dimensionText, userSettings);
    
    // Should detect exactly one conversion (the dimension), not multiple
    this.assert(allConversions.length === 1, 'Double Detection: Dimension format should yield 1 conversion', 1, allConversions.length);
    this.assert(allConversions[0].type === 'dimensions', 'Double Detection: Should detect as dimensions type', 'dimensions', allConversions[0].type);
    
    // Test case 2: Various dimension formats
    const testCases = [
      {
      ...
      },
      {
        ...
        description: 'Dimension in inches'
      },
      {
        text: 'Product: 2.5mm x 10mm x 50mm size',
        description: 'Small dimension in mm'
      }
    ];
    
    testCases.forEach((testCase, index) => {
      console.log(`Testing case ${index + 1}: ${testCase.description}`);
      const conversions = this.detector.findConversions(testCase.text, userSettings);
      
      // Should detect exactly one conversion
      this.assert(conversions.length === 1, 
        `Double Detection Case ${index + 1}: "${testCase.description}" should yield 1 conversion`, 
        1, conversions.length);
      
      if (conversions.length > 0) {
        this.assert(conversions[0].type === 'dimensions', 
          `Double Detection Case ${index + 1}: Should detect as dimensions type`, 
          'dimensions', conversions[0].type);
      }
    });
    
    // Test case 3: Mixed content with both dimensions and separate units (simplified)
    console.log('Testing mixed content with dimensions and separate units');
    const mixedText = 'Box size: 10 x 20 x 30 cm. Weight: 5 kg.';
    const mixedConversions = this.detector.findConversions(mixedText, userSettings);
    
    // Should detect 2 conversions: dimension and weight
    this.assert(mixedConversions.length === 2, 'Double Detection: Mixed content should yield 2 conversions', 2, mixedConversions.length);
    
    // Verify each conversion type
    const types = mixedConversions.map(c => c.type);
    this.assert(types.includes('dimensions'), 'Double Detection: Mixed content should include dimensions', 'dimensions included', types.join(', '));
    this.assert(types.includes('weight'), 'Double Detection: Mixed content should include weight', 'weight included', types.join(', '));
    
    // Test case 4: Edge cases that might cause overlapping
    const edgeCases = [
      {
        text: 'Size: 36.5x110.8x32cm (no spaces)',
        description: 'No spaces in dimension'
      },
      {
        text: 'Dimensions: 36.5 × 110.8 × 32 cm (multiply symbols)',
        description: 'Unicode multiply symbols'
      }
    ];
    ...
    edgeCases.forEach((testCase, index) => {
      ...
      // Should detect exactly one dimension
      this.assert(conversions.length === 1, 
        `Double Detection Edge Case ${index + 1}: Should yield 1 conversion`, 
        1, conversions.length);
      
      if (conversions.length > 0) {
        this.assert(conversions[0].type === 'dimensions', 
          `Double Detection Edge Case ${index + 1}: Should detect as dimensions`, 
          'dimensions', conversions[0].type);
      }
    });
    
    // Test case 5: Verify no false positives (simplified)
    const nonDimensionTexts = [
      'Height: 180 cm',
      'Single measurement: 25 inches'
    ];
    
    nonDimensionTexts.forEach((text, index) => {
      console.log(`Testing non-dimension case ${index + 1}`);
      const conversions = this.detector.findConversions(text, userSettings);
      
      this.assert(conversions.length === 1, 
        `Double Detection Non-Dimension ${index + 1}: Should detect single measurement`, 
        1, conversions.length);
      
      if (conversions.length > 0) {
        this.assert(conversions[0].type !== 'dimensions', 
          `Double Detection Non-Dimension ${index + 1}: Should NOT detect as dimensions`, 
          'not dimensions', conversions[0].type);
      }
    });
    
    console.log(`${colors.green}✓ Double detection prevention tests completed${colors.reset}`);
  }

  // Test specifically for the original double detection bug
  testOriginalDoubleDetectionBug() {
    console.log(`\n${colors.blue}[Testing Original Double Detection Bug Fix]${colors.reset}`);
    ...
    console.log('Testing the exact case from the original bug report...');
    
    // This was the exact example that was causing double detection
    const bugText = '36.5 x 110.8 x 32 cm';
    const conversions = this.detector.findConversions(bugText, userSettings);
    
    console.log(`Found ${conversions.length} conversion(s) for "${bugText}"`);
    if (conversions.length > 0) {
      console.log(`Conversion types: ${conversions.map(c => c.type).join(', ')}`);
      console.log(`First conversion: ${conversions[0].converted}`);
    }
    
    // The critical test: should yield exactly 1 conversion (the dimension), not 2 (dimension + single unit)
    this.assert(conversions.length === 1, 
      'Original Bug: "36.5 x 110.8 x 32 cm" should yield exactly 1 conversion (not double detection)', 
      1, conversions.length);
    
    this.assert(conversions[0].type === 'dimensions', 
      'Original Bug: Should detect as dimensions type only', 
      'dimensions', conversions[0].type);
    
    // Additional verification: the conversion should include all three dimensions
    if (conversions.length > 0) {
      const converted = conversions[0].converted;
      ...
      this.assert(hasAllDimensions, 
        'Original Bug: Conversion should include all three dimensions', 
        'all dimensions included', hasAllDimensions ? 'all included' : 'some missing');
    }
    
    console.log(`${colors.green}✓ Original double detection bug fix verified!${colors.reset}`);
  }
  ...
  async runAllTests() {
    ...
    this.testDoubleDetectionPrevention();
    this.testOriginalDoubleDetectionBug();
    this.testOriginalDoubleDetectionBug();
  }
}
